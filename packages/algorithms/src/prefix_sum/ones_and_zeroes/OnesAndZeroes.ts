...
function transformZeroes(nums: number[]) {
...
function generatePrefixSum(nums: number[]) {
    if (nums.length === 0) {
...
function findMaxLengthunderstand(nums: number[]): number {

    let maxLength = 0
    let firstPrefixSumOccurance = new Map<number, number>()
...
function findMaxLength(nums: number[]): number {
    ...
    let firstOcc: Record<string, number> = {
